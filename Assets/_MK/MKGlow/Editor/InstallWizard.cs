        ...
        private void OnGUI()
        {
            if(Configuration.isReady)
            {
                ...
                switch(_targetRenderPipeline)
                {
                    ...
                    EditorGUILayout.LabelField("1. Add MKGlow to your custom post processing under HDRP Default Settings on the “Before Post Process” List. Navigate to it via: Edit > Project Settings > HDRP Default Settings.", _flowTextStyle);
                    EditorGUILayout.LabelField(@"2. On your Volume Profile add Glow via “Post-processing/MK/MKGlow”", _flowTextStyle);
                    break;
                }
                VerticalSpace();
                Divider();
